...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, defaultMode = 'login' }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Google login redirects, so we don't need to close modal here usually
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2 hover:shadow-md transform hover:-translate-y-0.5"
          >
