...
class MemoryManager {
  ...
  getSystemContext(): string {
    const profile = this.getUserProfile();
    const memories = this.getMemories().slice(0, 20); // Last 20 memories

    let context = "";
    if (Object.keys(profile).length > 0) {
      context += `[USER_PROFILE: ${JSON.stringify(profile)}]\n`;
    }
    if (memories.length > 0) {
      context += `[RELEVANT_MEMORY: ${memories.map(m => m.content).join(' | ')}]\n`;
    }
    return context;
  }
}
